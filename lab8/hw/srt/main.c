...
void* threadFunction(void* arg){
  int start = (*((int*)arg))*partition;
  int end = *((int*)arg) != nthreads-1 ? start+partition: width;   // if this is the last thread, make sure that it does the last couple of pixels also
  Vec3 camera_pos;
...
main( int argc, char **argv )
{
    ...
    printf( "# Rendering scene with %d spheres and %d lights\n",
            scene.sphere_count,
            scene.light_count );
    partition = width / nthreads;
    int* start = (int*)malloc(sizeof(int)*nthreads);
    for(int i = 0; i < nthreads; i++){
      start[i]=i;
    }
    pthread_t* tid = (pthread_t*)malloc(sizeof(pthread_t)*nthreads);
    for(int i = 0; i < nthreads; i++){
      int ret = pthread_create(&tid[i], NULL, threadFunction, (void*)&start[i]); 
      if(ret){
